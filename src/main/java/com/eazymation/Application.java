...
import com.eazymation.clouvir.CasProperties;
import com.eazymation.clouvir.CustomAuthorizer;
import com.eazymation.clouvir.service.InvalidTokenService;
import com.eazymation.clouvir.service.UserService;
import com.eazymation.clouvir.util.SocketWrapper;
import com.eazymation.clouvir.util.upload.StorageProperties;
...
@Controller
@EnableScheduling
@ComponentScan(basePackages = { "vn.greenglobal.core.model.common", "com.eazymation.clouvir.controller",
		"com.eazymation.clouvir.service", "com.eazymation.clouvir" })
...
//		System.setProperty("user.timezone", "GMT+7");
    ...
    public ServerSocket serverSocket() throws IOException {
        return ServerSocketFactory.getDefault().createServerSocket(5995);
    }
            ...
            exec.execute(() -> {
                try {
                    while (true) {
                        ...
                        System.out.println("=========================================");
                        String externalIP = socket.getInetAddress().toString().replace("/", "");
                        
                        OutputStream os = socket.getOutputStream();
                        PrintWriter pw = new PrintWriter(new OutputStreamWriter(os, StandardCharsets.UTF_8), true);
                        BufferedReader br = new BufferedReader(new InputStreamReader(socket.getInputStream()));
                        String internalIP = br.readLine(); 
                        boolean checkExists = userService.checkExistsByIP(externalIP, internalIP);
                        System.out.println("externalIP: " + externalIP);
                        System.out.println("internalIP: " + internalIP);
                        String response = "";
                        if (checkExists) {
                        	socketWrapper.getSockets().put(externalIP + "/" + internalIP, socket);
                        	response= "mConnected";
                        	 pw.println(response);
                             pw.flush();
                        } else {
                        	response= "mFailed to detect user. Please contact to Administrator to config your IP.";
                        	pw.println(response);
                            pw.flush();
                            pw.close();
                            socket.close();
